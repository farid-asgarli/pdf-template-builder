...
> **WARNING:** The ShowEntire element imposes strict space constraints, which may lead to a DocumentLayoutException if the content exceeds the page's capacity. Ensure that the enclosed content fits within a single page to avoid errors.

> **TIP:** Please consider using a less-strict alternative, [EnsureSpace](/api-reference/ensure-space.html), if you want to maintain visual consistency without enforcing a hard page constraint.
